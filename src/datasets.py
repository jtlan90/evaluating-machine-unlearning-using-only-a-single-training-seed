...
CALTECH_MEAN = (0.485, 0.456, 0.406)
CALTECH_STD = (0.229, 0.224, 0.225)
...
# Caltech101-specific transforms using ImageNet normalization values
caltech_transform_train_from_scratch = [
    transforms.Resize((224, 224)),  # Use ImageNet standard size
    transforms.RandomHorizontalFlip(p=0.5),
    transforms.RandomRotation(degrees=15),
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
    transforms.ToTensor(),
    transforms.Lambda(lambda x: x.repeat(3, 1, 1) if x.size(0) == 1 else x),  # Convert grayscale to RGB
    transforms.Normalize(CALTECH_MEAN, CALTECH_STD),
]

caltech_transform_unlearning = [
