...
wandb.init(
    project=f"Methodology_Comparison_{args.net}_{args.dataset}_random_perc",
    name=f"{args.method}",
    group=f"Seed{args.seed}_{args.dataset}_random_{args.forget_perc}perc",
    tags=[f"Net:{args.net}", f"Dataset:{args.dataset}", f"ForgetClass:{args.forget_perc}perc", f"Method:{args.method}", f"Seed:{args.seed}"],
)
