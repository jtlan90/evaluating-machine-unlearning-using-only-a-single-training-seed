...
#############################################################
declare -a StringArray=("rocket" "mushroom" "baby" "lamp" "sea" "fox" "pine") # classes to iterate over

dataset=Cifar20
n_superclasses=20
n_subclasses=100

weight_path="./checkpoint/ViT/ViT-Cifar20-best.pth" # Add the path to your ViT weights
...
for val in "${StringArray[@]}"; do
    ...
    reset_cuda
    CUDA_VISIBLE_DEVICES=$DEVICE python forget_subclass_main.py -net ViT -dataset $dataset -superclasses $n_superclasses -subclasses $n_subclasses -gpu -method pdr_tuning_loss_free -forget_class $forget_class -weight_path $weight_path -seed $seed
    reset_cuda
