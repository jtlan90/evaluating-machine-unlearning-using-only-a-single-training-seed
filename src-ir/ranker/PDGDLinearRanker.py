...
from ranker.TwoLayerRanker import TwoLayerRanker
import numpy as np
import torch
...
class PDGDLinearRanker(LinearRanker):
    ...
    def get_query_result_list(self, dataset, query, flip_ranking = False, random=False):
        """
        ...
        if flip_ranking:
            np.random.shuffle(doc_scores)
        ranking = self._recursive_choice(np.copy(doc_scores),
                                         np.array([], dtype=np.int32),
                                         k,
                                         random) 
        selected_scores = doc_scores[ranking]

        return ranking, doc_scores, selected_scores
    ...
    def _recursive_choice(self, scores, incomplete_ranking, k_left, random):
        ...
        assert probs.ndim == 1, "Probs must be 1-dimensiona"
