...
from client.federated_optimize import train_uniform_model_poison
...
def do_task(task):
    ...
    train_result = train_uniform_model_poison(params=params, traindata=trainset, testdata=testset, message=task_info, num_update=num_update)
    return train_result
...
#         fold_id, click_model, ranker_id = task
#         save_path = path + "fold_{}/{}/result.npy".format(fold_id, click_model.name)
...
def run(path, tasks):
    ...
    for task, result in zip(tasks, results):
        fold_id, click_model, ranker_id = task
        save_path = path + "fold_{}/{}/result.pkl".format(fold_id, click_model.name) 
        if not os.path.exists(os.path.dirname(save_path)):
            os.makedirs(os.path.dirname(save_path))
    ...
    for seed in range(1, 2):
        for dataset in datasets:
            ...
            for fold_id in range(1):
                for ranker_id in range(1):
                    click_model = PERFECT_MODEL
