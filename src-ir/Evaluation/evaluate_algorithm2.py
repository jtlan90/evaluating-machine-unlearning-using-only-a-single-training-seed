...
def print_final_metrics(base_dir, dataset, seed_start, seed_end, save_dir):
    """Print final NDCG metrics with mean and std across training seeds"""
    scenarios = ['clean', 'data_poison', 'model_poison']
    models = ['Perfect', 'Navigational', 'Informational']
    methods = ['retrain', 'FedRemove', 'fedEraser', 'fineTuning', 'pga']
    
    os.makedirs(save_dir, exist_ok=True)
    output_path = os.path.join(save_dir, f"{dataset}_algorithm2_summary.txt")
    
    with open(output_path, 'w') as f:
        header = f"Algorithm 2 Results: {dataset}\n"
        header += f"Training/Unlearning seeds: {seed_start}-{seed_end}\n"
        header += "=" * 80 + "\n\n"
        f.write(header)
        print(header)
        ...
        for scenario in scenarios:
            ...
            for model in models:
                ...
                # Get training final NDCGs
                training_results = load_training_results(base_dir, dataset, scenario, model, 1,
                                                        seed_start, seed_end)
                if training_results is not None and len(training_results) > 0:
                    final_training = training_results[:, -1]
                    f.write(f"  Training final NDCG@10: {np.mean(final_training):.4f} ± {np.std(final_training):.4f}\n")
                    print(f"  Training final NDCG@10: {np.mean(final_training):.4f} ± {np.std(final_training):.4f}")
...
def compare_variance_sources(base_dir, dataset, seed_start, seed_end, save_dir):
    """Compare variance from training vs unlearning"""
    scenarios = ['clean', 'data_poison', 'model_poison']
    models = ['Perfect', 'Navigational', 'Informational']
    methods = ['retrain', 'FedRemove', 'fedEraser', 'fineTuning', 'pga']
    
    output_path = os.path.join(save_dir, f"{dataset}_variance_comparison.txt")
    
    with open(output_path, 'w') as f:
        header = "Variance Analysis: Pre-training Impact\n"
        header += "=" * 80 + "\n\n"
        f.write(header)
        print(header)
        ...
        for scenario in scenarios:
            ...
            for model in models:
                ...
                training_results = load_training_results(base_dir, dataset, scenario, model, 1,
                                                        seed_start, seed_end)
                if training_results is not None and len(training_results) > 0:
                    training_std = np.std(training_results[:, -1])
                    f.write(f"  Training Std Dev: {training_std:.4f}\n")
                    print(f"  Training Std Dev: {training_std:.4f}")
                
                # Unlearning variance for each method
                f.write(f"  {'Method':<15} {'Std Dev':<10}\n")
                print(f"  {'Method':<15} {'Std Dev':<10}")
                ...
                for method in methods:
                    ...
                    if unlearn_results is not None and len(unlearn_results) > 0:
                        unlearn_std = np.std(unlearn_results[:, -1])
                        line = f"  {method:<15} {unlearn_std:<10.4f}\n"
                        f.write(line)
                        print(line.rstrip())
    
    print(f"\nVariance comparison saved to {output_path}")

def calculate_w2_distances_final_algo2(base_dir, dataset, scenario, model, fold, seed_start, seed_end, methods):
    """Calculate W2 distances on final NDCG values for Algorithm 2"""
    scenario_map = {'clean': 'clean', 'data_poison': 'data', 'model_poison': 'model'}
    scenario_dir = scenario_map[scenario]
    ...
    retrain_finals = []
    for seed in range(seed_start, seed_end + 1):
        path = f"{base_dir}/{dataset}/{fold}/{scenario_dir}/{model}_training_seed{seed}_unlearning_retrain_seed{seed}_1000.pkl"
        try:
            with open(path, 'rb') as f:
                result = pickle.load(f)
                if hasattr(result, 'ndcg_server'):
                    retrain_finals.append(result.ndcg_server[-1])
        except FileNotFoundError:
            pass
...
def calculate_w2_trajectory_algo2(base_dir, dataset, scenario, model, fold, seed_start, seed_end, methods):
    """Calculate W2 distances over time for Algorithm 2"""
    scenario_map = {'clean': 'clean', 'data_poison': 'data', 'model_poison': 'model'}
    scenario_dir = scenario_map[scenario]
    ...
    retrain_results = np.array(retrain_results)
    w2_trajectories = {}
...
def print_w2_analysis_algo2(base_dir, dataset, seed_start, seed_end, save_dir):
    """Print W2 distance analysis comparing methods to retrain for Algorithm 2"""
    scenarios = ['clean', 'data_poison', 'model_poison']
    models = ['Perfect', 'Navigational', 'Informational']
    methods = ['retrain', 'FedRemove', 'fedEraser', 'fineTuning', 'pga']
    
    os.makedirs(save_dir, exist_ok=True)
    output_path = os.path.join(save_dir, f"{dataset}_algorithm2_w2_distances.txt")
    
    with open(output_path, 'w') as f:
        header = f"Wasserstein-2 Distance Analysis (Algorithm 2)\n"
        ...
        for scenario in scenarios:
            ...
            for model in models:
                w2_distances = calculate_w2_distances_final_algo2(
                    base_dir, dataset, scenario, model, 1, seed_start, seed_end, methods
                )
...
def plot_w2_trajectories_algo2(base_dir, dataset, scenario, seed_start, seed_end, save_dir):
    """Plot W2 distance evolution over time for Algorithm 2"""
    models = ['Perfect', 'Navigational', 'Informational']
    methods = ['FedRemove', 'fedEraser', 'fineTuning', 'pga']
    ...
    for j, model in enumerate(models):
        ...
        w2_trajectories = calculate_w2_trajectory_algo2(
            base_dir, dataset, scenario, model, 1, seed_start, seed_end, methods + ['retrain']
        )
    ...
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"{dataset}_{scenario}_algorithm2_w2_trajectory.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    print_final_metrics(args.base_dir, args.dataset, args.seed_start, args.seed_end, args.save_dir)
    
    print("\nAnalyzing variance sources...")
    compare_variance_sources(args.base_dir, args.dataset, args.seed_start, args.seed_end, args.save_dir)
    
    print("\n" + "=" * 80)
    print("Computing Wasserstein-2 distances...")
    print("=" * 80)
    print_w2_analysis_algo2(args.base_dir, args.dataset, args.seed_start, args.seed_end, args.save_dir)
    
    print("\nGenerating W2 trajectory plots...")
    for scenario in scenarios:
        print(f"  Plotting {scenario}...")
        plot_w2_trajectories_algo2(args.base_dir, args.dataset, scenario, 
                                   args.seed_start, args.seed_end, args.save_dir)
