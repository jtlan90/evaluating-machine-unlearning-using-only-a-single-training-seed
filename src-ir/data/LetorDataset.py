...
class LetorDataset(AbstractDataset):
    ...
    def _load_data(self):
        ...
        with open(self._path, "r") as fin:
            ...
            for line in fin:
                ...
                relevence = float(cols[0])  
                if relevence.is_integer():
                    relevence = int(relevence) 
    ...
    def update_label(self, qid, docid, new_label):

        if qid in self._query_docid_get_rel:
        
            if docid in self._query_docid_get_rel[qid]:
                self._query_docid_get_rel[qid][docid] = new_label
                docid_index = self._query_get_docids[qid].index(docid)
                self._query_relevant_labels[qid][docid_index] = new_label
       
    def get_docs_with_relevance_by_query(self, query):

        if query in self._query_get_docids:
            docids = self._query_get_docids[query]
            return [(docid, self._query_docid_get_rel[query][docid]) for docid in docids]
        else:
            return []