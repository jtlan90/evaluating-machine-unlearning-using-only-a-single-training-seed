...
def train(epochs):
    ...
    for batch_index, (images, _, labels) in enumerate(trainloader):
        ...
        # Add TensorBoard logging for training loss and learning rate
        writer.add_scalar('Train/loss', loss.item(), epoch * len(trainloader) + batch_index)
        writer.add_scalar('Train/lr', optimizer.param_groups[0]['lr'], epoch * len(trainloader) + batch_index)
...
def eval_training(epoch=0, tb=True):
    ...
    # Add TensorBoard logging for validation metrics
...
# dataloaders
# Dataset-specific root paths
if args.dataset == "PinsFaceRecognition":
    root = "105_classes_pins_dataset"
elif args.dataset == "Caltech101":
    root = "./data"  # Caltech101 will be downloaded to ./data/caltech101
else:
    root = "./data"
img_size = 224 if args.net == "ViT" or args.dataset == "Caltech101" else 32
...
# After creating the checkpoint_path, initialize SummaryWriter
checkpoint_path = os.path.join(conf.CHECKPOINT_PATH, args.net)
if not os.path.exists(checkpoint_path):
    os.makedirs(checkpoint_path)
checkpoint_path = os.path.join(checkpoint_path, "{net}-{dataset}-{type}.pth")

# Initialize TensorBoard writer
writer = SummaryWriter(log_dir=os.path.join("runs", args.net, conf.TIME_NOW))
