...
#############################################################
declare -a StringArray=("pine") # classes to iterate over
...
for val in "${StringArray[@]}"; do
    forget_class=$val
    echo "Starting parallel execution for forget_class: $forget_class"
    ...
    reset_cuda
    CUDA_VISIBLE_DEVICES=$DEVICE python forget_subclass_main.py -net ResNet18 -dataset $dataset -superclasses $n_superclasses -subclasses $n_subclasses -gpu -method pdr_tuning -forget_class $forget_class -weight_path $weight_path -seed $seed
    reset_cuda
    CUDA_VISIBLE_DEVICES=$DEVICE python forget_subclass_main.py -net ResNet18 -dataset $dataset -superclasses $n_superclasses -subclasses $n_subclasses -gpu -method pdr_tuning_loss_free -forget_class $forget_class -weight_path $weight_path -seed $seed
    reset_cuda
