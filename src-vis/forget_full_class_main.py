...
# Change alpha here as described in the paper for CIFAR20
if args.method == "ssd_tuning_loss_free" or args.method == "pdr_tuning_loss_free" and args.dataset == "Cifar20":
    model_size_scaler = 0.5
...
# Logging
wandb.init(
    project=f"Methodology_Comparison_{args.net}_{args.dataset}_fullclass",
    name=f"{args.method}",
    group=f"Seed{args.seed}_{args.dataset}_fullclass_{args.forget_class}",
    tags=[f"Net:{args.net}", f"Dataset:{args.dataset}", f"ForgetClass:{args.forget_class}", f"Method:{args.method}", f"Seed:{args.seed}"],
)
