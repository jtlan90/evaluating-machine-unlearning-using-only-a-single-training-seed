...
def amnesiac(
    ...
    **kwargs,
    ...
    """
    Implementation of random label unlearning based on the bad teacher unlearning approach.
    
    Note: This method is mislabeled as "amnesiac" but is actually random label unlearning.
    It assigns random labels from the remaining classes to forget samples, while retaining
    original labels for retain samples. The model is then trained on this modified dataset.
    
    Based on implementation from https://github.com/vikram2000b/bad-teaching-unlearning
    """
...
def FisherForgetting(
    ...
    **kwargs,
    ...
    # Implementation of Fisher Forgetting unlearning method for full class forgetting scenario
    def hessian(dataset, model):
        # Function to compute the Hessian (second-order derivatives) approximation for model parameters
        model.eval()
        # Set model to evaluation mode to disable dropout, etc.
        train_loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False)
        # Create a data loader with batch size 1 to process one sample at a time
        loss_fn = nn.CrossEntropyLoss()

        parameter_file = 'model_parameters.pt'
        parameter_dict = {f"param_{i}": p for i, p in enumerate(model.parameters())}
        torch.save(parameter_dict, parameter_file)
        ...
        #         f.write(f"Shape: {p.shape}\n")
        #         f.write(f"Data type: {p.dtype}\n")
        #         f.write(f"Device: {p.device}\n")
        #         f.write("Values:\n")
        #         # Convert tensor to numpy and write all values
        #         f.write(str(p.detach().cpu().numpy()))
        #         f.write("\n\n")
            
                
        #         # Exit after first parameter to avoid huge file
        #         break

    # Define cross-entropy loss function for classification
        for p in model.parameters():
            # Initialize accumulators for each parameter in the model
            # Initialize gradient accumulator to zero
            p.grad_acc = 0
            # print(p)
            # Initialize second-order gradient (Hessian diagonal) accumulator to zero
            p.grad2_acc = 0
            # print(p)

            # Initialize second-order gradient (Hessian diagonal) accumulator to zero
        # with open('parameters3.txt', 'w') as f:
        #     np.set_printoptions(threshold=np.inf)  # Set threshold to infinity
        ...
        #         f.write(f"Data type: {p.dtype}\n")
        #         f.write(f"Device: {p.device}\n")
        #         f.write(f"grad_acc: {p.grad_acc}\n")
        #         f.write(f"grad_acc2: {p.grad2_acc}\n")
        #         f.write("Values:\n")
        #         # Convert tensor to numpy and write all values
        #         f.write(str(p.detach().cpu().numpy()))
        #         f.write("\n\n")
        ...
        for data, _, orig_target in tqdm(train_loader):
            # Iterate through each sample in the dataset with progress bar
            data, orig_target = data.to(device), orig_target.to(device)
            # Move input data and target labels to the specified device (CPU/GPU)
            output = model(data)
            ...
            for y in range(output.shape[1]):
                ...
                # Create a new target tensor filled with the current class index
                loss = loss_fn(output, target)
                # Calculate loss between model output and the current class
                model.zero_grad()
                # Clear previous gradients
                loss.backward(retain_graph=True)
                # Compute gradients, keeping the computation graph for next iterations
                for p in model.parameters():
                    # For each parameter in the model
                    if p.requires_grad:
                        # Only process parameters that require gradients
                        p.grad_acc += (orig_target == target).float() * p.grad.data
                        # Accumulate gradients weighted by whether this is the true class
                        p.grad2_acc += prob[:, y] * p.grad.data.pow(2)
                        # Accumulate second-order gradients weighted by class probability
        
        
        # with open('parameters4.txt', 'w') as f:
        #     np.set_printoptions(threshold=np.inf)  # Set threshold to infinity
        ...
        #         f.write(f"Data type: {p.dtype}\n")
        #         f.write(f"Device: {p.device}\n")
        #         f.write(f"grad_acc: {p.grad_acc.cpu().numpy()}\n")
        #         f.write(f"grad_acc2: {p.grad2_acc.cpu().numpy()}\n")
        #         f.write("Values:\n")
        #         # Convert tensor to numpy and write all values
        #         f.write(str(p.detach().cpu().numpy()))
        #         f.write("\n\n")
        ...
        for p in model.parameters():
            # Normalize the accumulated gradients by dataset size
            p.grad_acc /= len(train_loader)
            # Compute average first-order gradient
            p.grad2_acc /= len(train_loader)
            # Compute average second-order gradient (diagonal Hessian approximation)

        # with open('parameters5.txt', 'w') as f:
        #     np.set_printoptions(threshold=np.inf)  # Set threshold to infinity
    ...
    def get_mean_var(p, is_base_dist=False, alpha=3e-6):
        # Calculate variance based on the inverse of the Hessian diagonal approximation
        var = deepcopy(1.0 / (p.grad2_acc + 1e-8))
        # Invert the Hessian diagonal with a small epsilon to avoid division by zero
        var = var.clamp(max=1e3)
        # Clamp variance to prevent extremely large values
        if p.size(0) == num_classes:
            # Special handling for the output layer (classifier)
            var = var.clamp(max=1e2)
            # Further restrict variance for the classifier layer
        var = alpha * var
        # Scale variance by the alpha hyperparameter

        if p.ndim > 1:
            # For multi-dimensional parameters (e.g., weight matrices)
            var = var.mean(dim=1, keepdim=True).expand_as(p).clone()
            # Average variance across one dimension and expand back to original shape
        if not is_base_dist:
            # For normal operation (not base distribution)
            mu = deepcopy(p.data0.clone())
            # Use the original parameter values as the mean
        else:
            # For base distribution (not used in this implementation)
            mu = deepcopy(p.data0.clone())
            # Same behavior as above (redundant code)
        if p.size(0) == num_classes:
            # Special handling for output layer weights corresponding to the forget class
            mu[forget_class] = 0
            # Zero out the mean for the forget class parameters
            var[forget_class] = 0.0001
            # Set very small variance for the forget class parameters
        if p.size(0) == num_classes:
            # Additional scaling for the output layer
            # Last layer
            var *= 10
            # Increase variance for the last layer by 10x
        elif p.ndim == 1:
            # For 1D parameters (e.g., bias terms or BatchNorm parameters)
            # BatchNorm
            var *= 10
            # Increase variance for BatchNorm parameters
        #         var*=1
        # Commented out code (no effect)
        return mu, var
        # Return mean and variance for parameter perturbation

    for p in model.parameters():
        # Store original parameter values before modification
        p.data0 = deepcopy(p.data.clone())
        # Create a deep copy of current parameters to use as reference

    hessian(retain_train_dl.dataset, model)
    # Calculate Hessian approximation using only the retain dataset

    fisher_dir = []

    
    # Initialize list to store Fisher directions for analysis
    alpha = 1e-6
    # Set alpha hyperparameter for scaling the variance
    for i, p in enumerate(model.parameters()):
        # Iterate through each parameter in the model
        mu, var = get_mean_var(p, False, alpha=alpha)
        # Get mean and variance for current parameter
        p.data = mu + var.sqrt() * torch.empty_like(p.data0).normal_()
        # Update parameter with random noise scaled by the square root of variance
        fisher_dir.append(var.sqrt().view(-1).cpu().detach().numpy())
        # Store the standard deviation (sqrt of variance) for analysis

    with open('parameters6.txt', 'w') as f:
        f.write(f"Parameter : Fisher data\n")
        f.write(f"data: {fisher_dir}\n")
    return get_metric_scores(
        # Evaluate the model after parameter perturbation
        model,
        unlearning_teacher,
        retain_train_dl,
        retain_valid_dl,
    ...
    # Return metrics to evaluate unlearning performance
...
def ssd_tuning(
    ...
    **kwargs,
    ...
    pdr = ssd_module.ParameterPerturber(model, optimizer, device, parameters)

    model = model.eval()

    # Initial calculation of importances and weight modification
...
def ssd_tuning_loss_free(
    ...
    **kwargs,
    ...
    # Calculation of the forget set importances
    sample_importances = pdr.calc_importance_loss_free(forget_train_dl)
    # print(sample_importances)

    # Calculate the importances of D (see paper); this can also be done at any point before forgetting.
    original_importances = pdr.calc_importance_loss_free(full_train_dl)
